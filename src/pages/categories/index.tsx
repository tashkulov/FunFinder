    ...
    return (
        <div className="min-h-screen bg-gray-100 px-4 py-10">
            ...
            <div className="max-w-5xl mx-auto grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {categories.map((category) => (<div key={category.id} onClick={() => handleSelectCategory(category)}
                        ...
                        <p className="text-gray-500">Нажмите, чтобы посмотреть события</p></div>))}</div>
            {selectedCategory && (<div className="max-w-3xl mx-auto mt-12 bg-white p-6 rounded-xl shadow"><h3 className="text-2xl font-semibold mb-4 text-indigo-600">
                    {selectedCategory.name} мероприятия</h3>{loading ? (<p className="text-gray-500">Загрузка...</p>
                    ) : events.length > 0 ? (
                        <ul className="space-y-4">{events.map((event) => (
                            <li key={event.id}
                                    className="p-4 rounded-lg bg-gray-100 hover:bg-indigo-100 transition-colors duration-200 shadow">
                                    <h4 className="text-lg font-medium">{event.title}</h4>
                                    <p className="text-sm text-gray-500">{event.date} | {event.location}</p></li>
                            ))}
                        </ul>
                    ) : (
